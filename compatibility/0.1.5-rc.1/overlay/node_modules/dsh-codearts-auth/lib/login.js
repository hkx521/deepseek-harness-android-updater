...
export function openBrowser(url) {
    ...
    // Android 平台专属适配：通过 3081 本地 HTTP 桥通知 MainActivity 唤起系统浏览器
    if (process.env.APP_NOTIFY_PORT || process.env.ANDROID_ROOT || process.platform === "android") {
        try {
            const notifyPort = process.env.APP_NOTIFY_PORT || 3081;
            const localToken = (process.env.APP_LOCAL_TOKEN || "").trim();
            const postData = JSON.stringify({ url });
            const req = request({
                hostname: "127.0.0.1",
                port: Number(notifyPort),
                path: "/open-url",
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Content-Length": Buffer.byteLength(postData),
                    "X-DSH-Token": localToken,
                },
                timeout: 5000,
            }, (res) => {
                res.resume();
            ...
            req.on("error", (err) => {
                console.warn("[codearts-auth] failed to send /open-url to bridge:", err);
            });
            req.on("timeout", () => {
                req.destroy();
            });
            ...
            console.error("[codearts-auth] failed to send /open-url via bridge; open manually:", url, error);
            return;
        }
    }
    ...
    try {
        ...
        child.on("error", (err) => {
            console.warn("[codearts-auth] open browser failed:", err);
        });
        child.unref();
    }
    catch (error) {
